...
def generate_prediction_report(run_id: str, report_data: dict, plot_paths: dict):
    ...
    # 7. Visualizations
    if plot_paths:
      if pdf.get_y() + 10 > pdf.page_break_trigger: # Check if enough space, add page if not
          pdf.add_page()
      ...
      if plot_paths.get("polynomial_plot"):
          pdf.add_plot_image(plot_paths["polynomial_plot"].replace("/static/", "static/"), f"Polynomial Regression (Degree {report_data.get('polynomial_degree', POLYNOMIAL_DEGREE)})") # Pass degree if available in report_data or use constant
    
    # Add Transformer Plots (NEW)
      if plot_paths.get("transformer_test_plot"):
          pdf.add_plot_image(plot_paths["transformer_test_plot"].replace("/static/", "static/"), "Transformer: Test Set Predictions")
      if plot_paths.get("transformer_future_plot"):
          pdf.add_plot_image(plot_paths["transformer_future_plot"].replace("/static/", "static/"), "Transformer: All Predictions")

      if plot_paths.get("polynomial_plot"):
          degree = report_data.get("polynomial_results",{}).get("degree",POLYNOMIAL_DEGREE) # Get degree if stored, else default
          pdf.add_plot_image(plot_paths["polynomial_plot"].replace("/static/", "static/"), f"Polynomial Regression (Degree {degree})")
